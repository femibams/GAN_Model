"""Inference: generate a single sample face from a trained checkpoint."""
import argparse
import os
...
def parse_args():
    p = argparse.ArgumentParser(description="Generate a face from a trained StyleGAN2-small checkpoint")
    p.add_argument("--ckpt", type=str,
                   default=os.path.join("outputs", "checkpoints", "latest.pth"),
                   help="Path to checkpoint produced by train.py")
    p.add_argument("--out", type=str, default=os.path.join("outputs", "sample.png"),
...
def _save_single_image(img: torch.Tensor, path: str) -> None:
    """Save a single [-1, 1] CHW tensor as a PNG (no grid padding)."""
    os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    arr = img.detach().clamp(-1, 1).add(1).div(2).mul(255).add_(0.5).clamp_(0, 255).to(torch.uint8).cpu()
    if arr.shape[0] == 1:
        arr = arr.expand(3, -1, -1)
    arr = arr.permute(1, 2, 0).contiguous()
    h, w, _ = arr.shape
    Image.frombytes("RGB", (w, h), bytes(arr.flatten().tolist())).save(path)
...
def main():
    ...
    _save_single_image(imgs[0], args.out)
